...
public class TurnonResource implements OcPlatform.EntityHandler {
    private static final String TAG = TurnonResource.class.getSimpleName();
    ...
    public TurnonResource(Gpio ledGpio) {
    ...
    private EntityHandlerResult handlePostRequest(OcResourceRequest request) {
        ...
        response.setResourceHandle(request.getResourceHandle());
        try {
            Log.d(TAG, "what: " + request.getResourceRepresentation().getValue("what"));
        } catch (OcException e) {
            e.printStackTrace();
        }
        response.setResourceRepresentation(getOcRepresentation());
    ...
    private OcRepresentation getOcRepresentation() {
